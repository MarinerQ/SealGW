...
def generate_random_inject_paras(Nsample, spin_type='aligned', dmin, dmax, m1_low,m1_high,q_low, a_max, m2_low):
...
if __name__ == '__main__':
    ...
    det_flag = str(sys.argv[2])
    source_type = str(sys.argv[3])

    core_num = int(sys.argv[-1])
    

    if source_type == 'BNS':
        samples = generate_random_inject_paras(Nsample=Nsample,dmin = 100, dmax=400,m1_low=1.1,m1_high=2, q_low = 0.8, a_max=0.1, m2_low=1.1)
    elif source_type == 'BBH':
        samples = generate_random_inject_paras(Nsample=Nsample,dmin = 100, dmax=4000,m1_low=6,m1_high=90, q_low = 0.25, a_max=0.1, m2_low=6)
    elif source_type == 'NSBH':
        samples = generate_random_inject_paras(Nsample=Nsample,dmin = 100, dmax=500,m1_low=6,m1_high=90, q_low = 0.1, a_max=0.1, m2_low=1.1)
    else:
